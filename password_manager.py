...
def main(): 
    ...
    if authenticated:
        ...
        # Close connection to sqlite3 db and encrypt the entire database file
        def close_and_encrypt():
            conn.close()
            with open('db1.db', 'rb') as f:
                data = f.read()
            key = generate_key()
            fernet = Fernet(key)
            encrypted_data = fernet.encrypt(data)
            with open('db1.db', 'wb') as f:
        ...
        messagebox.showerror("Error","Cannot authenticate.")
        root.quit()

# Register close_and_encrypt function to execute upon closing the root window
# root.protocol("WM_DELETE_WINDOW", close_and_encrypt)
