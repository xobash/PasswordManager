# Working on adding error handling as well as fixing the authentication error from auth_user.
...
class PasswordManagerApp:
    ...
    def password_generator(self):
        try:
            middle_frame = self.middle_frame # Define middle frame here. 
            background_color = "#0d1117" if self.dark_mode else "white"
            foreground_color = "#c9d1d9" if self.dark_mode else "black"
            accent_color = "#21262d" if self.dark_mode else "light gray"
...
def main():
    try:
        from auth_user import AuthUser # import here to avoid circular import.
        ...
        # Create root window, wont work without this for some reason.
        root = tk.Tk()
        ...
        if auth_user.check_authentication():
            # Launch PasswordManagerApp
            app = PasswordManagerApp(root)
            root.mainloop()
        else:
            # If authentication fails, destroy root window
            root.destroy()
